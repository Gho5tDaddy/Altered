...
If the review displays **Needs Setup**, Altered found a feat, subclass feature, attuned item, spell, or homebrew option whose full rules are not in the shared SRD pack. Open **Complete Private Mechanics**, use the authorized D&D Beyond character page as your reference, and enter only a short table-ready summary in your own words. Supported values such as a Speed bonus, resistance, immunity, Armor Class formula, or activated transformation can then affect play. Conditional or complex options remain clearly labeled reminders instead of being guessed. Each completion is stored as an existing schema-v1 private pack matched to that character and reapplies automatically on that device.

This workflow also works for content made available through a D&D Beyond campaign when the user is authorized to view it. It does not copy a book, bypass access controls, scrape a protected page, or ask for D&D Beyond credentials. Altered imports the public character facts, while the user privately confirms only the mechanics needed during play.
