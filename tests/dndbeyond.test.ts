...
test('imports Astral Arms as an activatable enhancement instead of guessing live state',()=>{
  ...
  const report=importDdbCharacter(payload,'152187683'),grant=report.character.transformationGrants?.find(entry=>entry.id==='ddb-astral-arms'),visage=report.character.transformationGrants?.find(entry=>entry.id==='ddb-astral-visage'),combined=report.character.transformationGrants?.find(entry=>entry.id==='ddb-astral-arms-visage');assert.ok(grant);assert.equal(grant.profile,'overlay');assert.equal(grant.resourceId,'focus-points');assert.equal(report.character.resources.find(entry=>entry.id==='focus-points')?.current,1);assert.equal(grant.effects?.activationActions?.[0]?.damageOnFail?.[0]?.expression,'2d8');const strike=grant.effects?.actions?.find(entry=>entry.id==='astral-arms-unarmed-strike');assert.ok(strike?.type==='attack');assert.equal(strike.ability,'wis');assert.equal(strike.reach,10);assert.equal(strike.damage[0]?.type,'Force');assert.match(strike.damage[0]?.expression??'',/^1d8/);assert.equal(grant.effects?.attackAbilityOverride,undefined);assert.ok(visage);assert.deepEqual(visage.effects?.skillAdvantage,['Insight','Intimidation']);assert.equal(combined?.resourceCost,2);assert.equal(combined?.effects?.actions?.length,2);assert.equal(report.setupNeeds.some(entry=>/arms of the astral self|visage of the astral self/i.test(entry.label)),false);
});
