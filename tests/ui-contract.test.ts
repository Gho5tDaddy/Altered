...
test('phone gameplay uses focused views while keeping the form artwork persistent',()=>{
  ...
  assert.match(html,/class="persistent-form-visual"[\s\S]*id="persistent-form-name"[\s\S]*id="form-art"[\s\S]*id="persistent-total-hp"[\s\S]*id="persistent-effects-summary"/);
...
test('cockpit effect names stay concise and the full HP breakdown can wrap',()=>{
  ...
  assert.equal(/id="open-active-effects"/.test(html),false);assert.equal(/#open-active-effects/.test(source),false);
  assert.match(source,/#persistent-effects-summary'[\s\S]*openTask\('features','play'\)[\s\S]*#active-effects/);
});
...
test('focused action pages prioritize executable controls without losing advanced choices',()=>{
  ...
  assert.match(source,/Single attacks & alternatives/);assert.match(source,/multiattack-components/);
