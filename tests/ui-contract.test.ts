...
test('private PDFs use account-scoped hosted storage without weakening structured pack validation',()=>{
  ...
  assert.match(html,/up to 500 MB/);assert.match(source,/PRIVATE_PDF_LIMIT=500\*1024\*1024/);assert.match(source,/PRIVATE_PDF_PART_SIZE=10\*1024\*1024/);assert.match(source,/action=create/);assert.match(source,/action=part/);assert.match(source,/action=complete/);assert.match(source,/Use for Setup/);assert.match(source,/credentials:'same-origin'/);
  assert.equal(hosting.r2,'PRIVATE_FILES');assert.match(worker,/maxPrivatePdfBytes=500\*1024\*1024/);assert.match(worker,/createMultipartUpload/);assert.match(worker,/resumeMultipartUpload/);assert.match(worker,/maxPrivatePdfParts/);assert.match(worker,/privatePdfPrefix\(user\)/);assert.match(worker,/env\?\.PRIVATE_FILES/);assert.match(worker,/oai-authenticated-user-id/);assert.match(worker,/private, no-store/);
