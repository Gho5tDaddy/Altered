...
For a normal Windows installation, download and run `Altered-Windows-Setup-v0.29.2.exe`. It installs for the current Windows user, adds an Altered shortcut to the Desktop and Start menu, uses the moon/runic A icon, and includes an uninstaller. The installer is not code-signed, so Windows can show an Unknown publisher warning.
