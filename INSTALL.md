...
For a normal Windows installation, download and run `Altered-Windows-Setup-v0.29.15.exe`. It installs for the current Windows user, adds an Altered shortcut to the Desktop and Start menu, uses the moon/runic A icon, and includes an uninstaller. The main shortcut opens the secure hosted app so linked-character Refresh and future updates work; the Start menu also contains an **Altered Offline** fallback. The installer is not code-signed, so Windows can show an Unknown publisher warning.

For a portable Windows or macOS copy, extract `Altered-Desktop-Mac-v0.29.15.zip`, keep all included files together, then open `Altered-v0.29.15.html` in a current browser. The package includes Ferocitus, the rules engine, Help, all six built-in form images, the no-JSON customization tools, and the lazy-loaded PDF/OCR readers.
