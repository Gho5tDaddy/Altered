...
export function availableTransformations(character:Character,state?:GameState):TransformationOption[]{
  ...
  for(const grant of character.transformationGrants??[]){
    ...
    const resourcePool=grant.resourceId&&state?resource(state,grant.resourceId):undefined,resourceName=resourcePool?.name;const resourceCost=grant.resourceCost??1;const resourceReason=resourcePool&&resourceCost>1?`Needs ${resourceCost} ${resourceName}; ${resourcePool.current} ${resourcePool.current===1?'remains':'remain'}.`:`No ${resourceName??grant.resourceId??'required resource'} remain.`;const usable=activeOverlay||resourceReady;const common={grantId:grant.id,source:grant.source,actionCost:activeOverlay?(grant.endActionCost??'none'):grant.actionCost,usable,...(!usable?{reason:resourceReason}:{}),duration:grant.duration,endActionCost:grant.endActionCost,resourceId:grant.resourceId,resourceCost:grant.resourceCost,concentration:grant.concentration,spellName:grant.spellName,spellLevel:grant.spellLevel,switchGroup:grant.switchGroup,retention:grant.retention,effects:grant.effects,...(activeOverlay?{deactivate:true}:{})};
...
function resolveSkills(character:Character,state:GameState,option:TransformationOption,abilities:Character['abilities'],effects:TransformationEffects[]):Record<string,DerivedRoll>{
  ...
  for(const name of names){
    ...
    const advantageSources=effects.filter(e=>e.checkAdvantage?.includes(a)||e.skillAdvantage?.some(skill=>sameText(skill,name))).map(()=> 'Active transformation');const disadvantageSources=effects.filter(e=>e.checkDisadvantage?.includes(a)||e.skillDisadvantage?.some(skill=>sameText(skill,name))).map(()=> 'Active transformation');if(conditionApplies(state,'Poisoned',conditionImmunities))disadvantageSources.push('Poisoned');const conditionalSources=conditionApplies(state,'Frightened',conditionImmunities)?['Frightened imposes Disadvantage only while the source is in line of sight.']:[];
...
function resolvedActions(character:Character,state:GameState,option:TransformationOption,effects:TransformationEffects[],canAttack:boolean,abilities:Character['abilities']):CreatureAction[]{
  ...
  if(canAttack&&character.feats.some(feat=>sameText(feat,'Sentinel'))&&retainedClassFeatures(option)){
    const trigger='A creature within 5 feet of you just took the Disengage action or hit a target other than you with an attack.';
    const eligible=adjusted.filter(action=>action.type==='attack'&&(action.kind==='weapon'||action.kind==='unarmed'||action.kind==='beast')&&!action.range) as Extract<CreatureAction,{type:'attack'}>[];
    for(const attack of eligible)adjusted.push({...attack,id:`sentinel-${attack.id}`,name:`Sentinel — ${attack.name}`,cost:'reaction',prerequisite:trigger,effects:[...(attack.effects??[]),{condition:'Speed 0',duration:'Rest of the current turn',note:'Sentinel: Halt'}],notes:[attack.notes,'Sentinel Opportunity Attack. Confirm the Guardian trigger before rolling; on a hit, Halt makes the creature’s Speed 0 for the rest of the current turn.'].filter(Boolean).join(' · ')});
  }
