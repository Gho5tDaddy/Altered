...
const PACKS=[
  {metadata:metadata({id:'altered-srd-creatures',name:'SRD 5.2.1 Creature Forms',version:'0.18.0',ruleset:RULES_VERSION.label,domain:'creatures',priority:100,license:'CC BY 4.0 source data; original Altered presentation',source:'Official SRD 5.2.1 / 2024 Basic Rules',verified:RULES_VERSION.reviewed,builtIn:true,description:'Verified creature stat blocks available to bundled transformations.'}),records:records(RAW_CREATURES)},
  {metadata:metadata({id:'altered-core-class-features',name:'Core Class Interaction Rules',version:'0.18.0',ruleset:RULES_VERSION.label,domain:'class-features',priority:100,license:'Mechanical summaries authored for Altered',source:'Official 2024 class rules',verified:RULES_VERSION.reviewed,builtIn:true,description:'Structured transformation-relevant mechanics for SRD/core classes; additional classes and subclasses can be supplied by private packs.'}),records:featureRecord(RAW_CLASS_FEATURES)},
  {metadata:metadata({id:'altered-core-species-features',name:'Core Species Interaction Rules',version:'0.18.0',ruleset:RULES_VERSION.label,domain:'species-features',priority:100,license:'Mechanical summaries authored for Altered',source:'Official 2024 character origins',verified:RULES_VERSION.reviewed,builtIn:true,description:'Structured transformation-relevant species behavior.'}),records:featureRecord(RAW_SPECIES_FEATURES)},
  {metadata:metadata({id:'altered-core-feat-hooks',name:'Recognized Feat Hooks',version:'0.18.0',ruleset:RULES_VERSION.label,domain:'feats',priority:100,license:'Mechanical summaries authored for Altered',source:'Character import hooks',verified:RULES_VERSION.reviewed,builtIn:true,description:'Minimal feat identifiers currently evaluated by the rules engine.'}),records:records(featEntries)},
  {metadata:metadata({id:'altered-spell-hooks',name:'Transformation Spell Hooks',version:'0.18.0',ruleset:RULES_VERSION.label,domain:'spells',priority:100,license:'Mechanical summaries authored for Altered',source:'Character import and transformation profiles',verified:RULES_VERSION.reviewed,builtIn:true,description:'Spell identifiers used by the engine; full spell data remains imported or supplied by licensed packs.'}),records:records(spellEntries)},
  {metadata:metadata({id:'altered-item-hooks',name:'Item Rule Hooks',version:'0.18.0',ruleset:RULES_VERSION.label,domain:'items',priority:100,license:'Altered schema',source:'Character import',verified:RULES_VERSION.reviewed,builtIn:true,description:'Reserved item domain for equipment and magic-item extension packs.'}),records:records(itemEntries)},
  {metadata:metadata({id:'altered-core-conditions',name:'Core Conditions',version:'0.18.0',ruleset:RULES_VERSION.label,domain:'conditions',priority:100,license:'Mechanical summaries authored for Altered',source:'Official 2024 rules glossary',verified:RULES_VERSION.reviewed,builtIn:true,description:'Condition records used by combat-state evaluation.'}),records:CONDITIONS},
  {metadata:metadata({id:'altered-transformation-profiles',name:'Transformation Profiles',version:'0.18.0',ruleset:RULES_VERSION.label,domain:'transformation-profiles',priority:100,license:'Mechanical summaries authored for Altered',source:'Official transformation rules',verified:RULES_VERSION.reviewed,builtIn:true,description:'Replacement and retention policies for each supported transformation type.'}),records:TRANSFORMATION_PROFILES}
