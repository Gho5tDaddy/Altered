...
function renderActiveEffects(){
  ...
  const names=cards.map(card=>card.querySelector('strong')?.textContent).filter((name):name is string=>Boolean(name));root.dataset.hasEffects=String(cards.length>0);root.hidden=cards.length===0||currentTab!=='features';if(cards.length){const summary=document.createElement('summary');const copy=document.createElement('span');copy.append(text('strong',`Active effects (${cards.length})`),text('small',names.join(' · ')));summary.append(copy);root.append(summary,...cards);root.open=wasOpen;}
...
function renderActions(){
  ...
  const unavailable=document.createElement('div');unavailable.className='unavailable-action-list';let availableCount=0,unavailableCount=0;const appendAction=(node:HTMLElement,blocked:boolean)=>{if(blocked){unavailable.append(node);unavailableCount++;}else{root.append(node);availableCount++;}};
  for(const action of sheet.actions){
    ...
    if(action.type==='attack'){
      ...
      collapseActionOptions(c.options);const use=button(economyError??`Roll ${action.name} Dice`,()=>resolveMultiattack(action),economyError?'button secondary action-roll':'button primary action-roll');use.disabled=Boolean(economyError);c.actions.append(use);appendAction(c.node,Boolean(economyError));
    }else{
      const limit=limitedActionStatus(action);const blocked=limit.unavailable||Boolean(economyError);const detail=[action.prerequisite,action.damage?.length?`${action.damageTiming??'Listed damage'}: ${action.damage.map(packet=>`${packet.expression} ${packet.type}`).join(' + ')}`:'',action.effects?.length?`Effects: ${effectsText(action.effects)}`:'',actionLimitText(action),action.notes,economyError].filter(Boolean).join(' · ');const c=card(action.name,limit.recharge?'Recharging':limit.remaining===0?'Expended':actionCostLabel(action.cost),detail,blocked?(limit.unavailable?'locked':'unavailable'):'available');if(blocked)c.badge.className='badge inactive';const label=limit.recharge?`Awaiting ${limit.recharge.min}–${limit.recharge.max}`:limit.remaining===0?'No uses remaining':economyError??`Use ${action.name}`;const use=button(label,()=>resolveAutomaticAction(action),blocked?'button secondary action-roll':'button primary action-roll');use.disabled=blocked;c.actions.append(use);appendAction(c.node,blocked);
    }
  }
  if(unavailableCount){const details=document.createElement('details');details.className='unavailable-actions';details.open=availableCount===0;const summary=document.createElement('summary');summary.textContent=`Unavailable right now (${unavailableCount})`;details.append(summary,unavailable);root.append(details);}
}
