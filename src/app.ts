import type {Ability,ActionCost,AttackAction,AutomaticActionChoice,Character,ConditionEffect,CreatureAction,DamagePacket,DamageType,EvaluatedFeature,GameState,OverlayTiming,OwnedContentMatch,OwnedContentPack,ReceivedEffect,ReceivedEffectKind,ResolvedSheet,Spell,TransformationEffects,TransformationOption,TransitionResult} from './types.js';
...
import {installExtensionPack,listExtensionPackRecords,loadArtOverride,loadBooleanSetting,loadJsonSetting,optimizePortrait,removeArtOverride,removeExtensionPack,removeSetting,saveArtOverride,saveBooleanSetting,saveJsonSetting,setStorageFailureHandler} from './storage.js';
...
import type {SpellCastingOption} from './engine.js';
import {
  actionCostError,actionExecutionError,addReceivedEffect,applyCondition,applyDamage,attackBonuses,attackCountsAsUnarmedStrike,attackRollSources,availableSpellCastingOptions,availableTransformations,boundedWhole,castSpell,criticalDiceExpression,criticalHitThreshold,
  actionSurgeError,clearConditions,completeTruePolymorph,concentrationSaveMode,createInitialState,deathSaveMode,declareAttack,declareRecklessAttack,endConcentration,endRage,endReceivedEffect,endSpellEffect,endTransformation,endTurn,
  extendRage,heal,longRest,markActionRechargeUsed,markLimitedActionUsed,markOncePerTurn,monkDie,pendingActionRecharge,remainingActionUses,removeCondition,resolveAdvantage,resolveConcentrationCheck,resolveDeathSave,resolveRelentlessRage,resolveSheet,resolveTempHpChoice,restoreDragonWings,rollDice,
  layOnHandsError,proficiencyBonus,rageStartError,recklessAttackError,rollAttackD20,rulesMetadata,secondWindError,selectedOptionalDamagePackets,shortRest,spellActiveEffect,spendActionCost,spendActionExecution,startCombat,startNewTurn,startRage,startTransformation,useActionSurge,useLayOnHands,useSecondWind,useWildResurgence,wildResurgenceError
} from './engine.js';
...
const selectedSpellCastingOptions=new Map<string,string>();
...
let latestD20Context:{result:D20Result;modifier:number;criticalThreshold:number}|undefined;
let pendingHeroicRoll:{result:D20Result;modifier:number;criticalThreshold:number;displayAdjustment:number;title:string}|undefined;
...
let invalidPackRecords:Array<{id:string;pack:unknown;reason:string}>=[];
...
function setAppMenuOpen(open:boolean,restoreFocus=false){
  ...
  if(open)setTimeout(()=>{const first=Array.from($('#top-actions').querySelectorAll<HTMLElement>('button:not([disabled]),a[href]')).find(control=>!control.hidden&&control.offsetParent!==null);first?.focus({preventScroll:true});},0);
  else if(restoreFocus)toggle.focus({preventScroll:true});
}
const TASK_TITLES:Record<string,string>={actions:'Actions',reactions:'Reactions',rolls:'Saves & Skills',spells:'Spells',features:'Abilities & Features',equipment:'Equipment & Inventory',rules:'Rules & Defenses'};
function syncWorkspaceChrome(){
  ...
  $('#character-snapshot').hidden=taskOrigin!=='sheet';
...
function showStorageWarning(message:string){const warning=document.querySelector<HTMLElement>('#storage-warning');if(!warning)return;warning.hidden=false;$('#storage-warning-copy').textContent=`${message} Export your character now so your work is recoverable.`;}
function persist(){try{localStorage.setItem('altered-v0.18',JSON.stringify({storageSchemaVersion:1,baseCharacters,currentCharacterId:baseCharacter.id,state,deletedCharacterIds:[...deletedCharacterIds]}));}catch{showStorageWarning('Altered could not save the current character and combat state on this device.');}}
function finishFirstCharacterSetup(){firstRunCharacterSetup=false;try{localStorage.setItem(FIRST_CHARACTER_SETUP_KEY,'complete');}catch{showStorageWarning('Altered could not remember that first-time setup is complete.');}}
...
function savedOverlayTimings(value:unknown,retainedIds:string[],fallbackTurn:number):Record<string,OverlayTiming>{
  if(typeof value!=='object'||value===null||Array.isArray(value))return {};
  const retained=new Set(retainedIds),entries:Array<[string,OverlayTiming]>=[];
  for(const [id,raw] of Object.entries(value).slice(0,20)){
    if(!retained.has(id)||typeof raw!=='object'||raw===null||Array.isArray(raw))continue;
    const item=raw as {startedTurn?:unknown;duration?:unknown;label?:unknown;tempHpSource?:unknown;concentrationName?:unknown};
    if(typeof item.startedTurn!=='number'||typeof item.duration!=='string'||typeof item.label!=='string')continue;
    const timing:OverlayTiming={startedTurn:boundedWhole(item.startedTurn,fallbackTurn,1,1_000_000),duration:item.duration.slice(0,200),label:item.label.slice(0,120)};
    if(typeof item.tempHpSource==='string')timing.tempHpSource=item.tempHpSource.slice(0,120);
    if(typeof item.concentrationName==='string')timing.concentrationName=item.concentrationName.slice(0,120);
    entries.push([id,timing]);
  }
  return Object.fromEntries(entries);
}
function restore(){
  try{
    ...
    const parsed=JSON.parse(raw) as {storageSchemaVersion?:unknown;baseCharacters?:unknown[];currentCharacterId?:string;baseCharacter?:unknown;character?:unknown;state?:Partial<GameState>;deletedCharacterIds?:unknown[]};
    if(parsed.storageSchemaVersion!==undefined&&parsed.storageSchemaVersion!==1)restoredDataRepairs.push('This save used an unfamiliar storage version; Altered recovered every compatible field without overwriting the original browser record.');
    ...
    if(saved){clean.hp=boundedWhole(saved.hp,clean.hp,0,restored.hp.max);clean.tempHp=boundedWhole(saved.tempHp,0,0,9999);
      ...
      for(const [id,pool] of Object.entries(saved.resources??{})){const canonicalId=['focus','ki','ki-points'].includes(id.toLowerCase())?'focus-points':id;const target=clean.resources[canonicalId];if(target&&pool&&typeof pool==='object'){const restoredCurrent=boundedWhole((pool as {current?:unknown}).current,target.current,0,target.max);target.current=canonicalId==='focus-points'?Math.min(target.current,restoredCurrent):restoredCurrent;}}
      ...
      clean.overlayTimings=savedOverlayTimings(saved.overlayTimings,clean.overlays,clean.turn.number);
      if(saved.rage&&typeof saved.rage==='object'&&typeof saved.rage.active==='boolean'){const active=saved.rage.active,startedAtTurn=active?boundedWhole(saved.rage.startedAtTurn,clean.turn.number,1,clean.turn.number):0,legacyEnd=boundedWhole(saved.rage.endsAtTurn,clean.rage.endsAtTurn,0,1_000_000);clean.rage={active,startedAtTurn,endsAtTurn:active?Math.min(legacyEnd||clean.turn.number+1,startedAtTurn+100):0,usedThisTurn:Boolean(saved.rage.usedThisTurn),recklessDeclared:Boolean(saved.rage.recklessDeclared),extendedThisTurn:Boolean(saved.rage.extendedThisTurn)};}
...
function resetLatestResult(){latestRollTab='actions';latestRollTitle='Ready';latestD20Context=undefined;pendingHeroicRoll=undefined;const title='Ready',total='—',detail='Press an attack, spell, save, or skill button. Altered rolls the correct dice and modifiers automatically.';$('#latest-roll').classList.remove('flash');$('#roll-title').textContent=title;$('#roll-total').textContent=total;$('#roll-detail').textContent=detail;$('#play-roll-title').textContent=title;$('#play-roll-total').textContent=total;$('#play-roll-detail').textContent=detail;dismissRollToast();}
function setCharacter(next:Character){character=next;baseCharacter=baseCharacters.find(entry=>entry.id===next.id)??next;state=createInitialState(character);sheet=resolveSheet(character,state);selectedOptionId='base';currentTab='actions';formSearch='';formFilter='all';$<HTMLInputElement>('#form-search').value='';$<HTMLSelectElement>('#form-filter').value='all';radiantActions.clear();selectedOptionalBonuses.clear();selectedRollModes.clear();selectedMultiattackVariants.clear();selectedSpellCastingOptions.clear();pendingDamageRolls.clear();resetLatestResult();notify(`${character.name} loaded in Base Form.`);render();}
function reconcileState(next:Character,previous:GameState,syncSheetResources=false){
  ...
  for(const [id,pool] of Object.entries(clean.resources)){const candidates=[previous.resources[id],...(id==='focus-points'?[previous.resources.focus,previous.resources['ki-points']]:[])].filter((entry):entry is NonNullable<typeof entry>=>Boolean(entry));if(candidates.length&&!syncSheetResources)pool.current=Math.min(...candidates.map(entry=>entry.current),pool.max);}
...
function renderSettings(){
  ...
  $('#content-registry-summary').textContent=`${auditSnapshot.rules} source-ledgered rules cover ${auditSnapshot.functions} state-changing functions (${auditSnapshot.counts.calculated} calculated, ${auditSnapshot.counts.conditional} conditional). Built-in packs are verified through ${registrySnapshot.verifiedThrough}; ${installedPacks.length} private user-reviewed pack${installedPacks.length===1?' is':'s are'} installed.`;
...
async function reviewAssistantProposal(file:File){
  ...
  try{
    ...
    status.textContent=result.applied?`${pack.metadata.name} passed Altered’s structural checks and was added to ${character.name}. ${packCounts(pack)}. Source claims were not fetched by Altered; review every visible control against your 2024 source before play.`:`${pack.metadata.name} has valid structure but does not match ${character.name}; nothing changed.`;
    renderSettings();
...
function syncPrivatePdfReviewButton(){const review=pendingPrivatePdfReview;const count=review?.matches.filter(match=>match.selected).length??0;const apply=$<HTMLButtonElement>('#apply-private-pdf-matches');apply.disabled=count===0;apply.textContent=count?`Add ${count} Reference ${count===1?'Reminder':'Reminders'}`:'No Matches Selected';}
async function applyPrivatePdfMatches(){
  ...
  for(const match of selected){const pack=privateMechanicPack(baseCharacter,{packId:`pdf-${slug(review.record.id)}-${slug(match.id)}`.slice(0,120),name:match.name,source:`Private PDF: ${review.record.name}, page ${match.page}`,summary:match.summary,mode:'reference',retainInWildShape:false,activation:'none'});await installExtensionPack(pack);}
  installedPacks=await loadValidatedInstalledPacks();rebuildEffectiveCharacterLibrary(true);renderInstalledPacks();renderSettings();render();$<HTMLDialogElement>('#private-pdf-review-dialog').close();privatePdfStatus(`${selected.length} private reference ${selected.length===1?'reminder':'reminders'} added for ${character.name}.`);notify(`Private reference reminders added for ${character.name}.`);showPrivatePdfResult('Reference reminders added',`${selected.length} matching private-source ${selected.length===1?'reminder was':'reminders were'} added for ${character.name}. No spell, item, statistic, action, or Wild Shape rule was automated without structured confirmation.`,[...selected.map(match=>`${match.name} — ${match.kind}, page ${match.page}`),`Use Advanced setup when a reminder needs an executable control. ${installedPacks.length} private ${installedPacks.length===1?'pack is':'packs are'} installed on this device.`]);pendingPrivatePdfReview=null;
}
...
async function refreshLinkedCharacter(manual=false,force=false){
  ...
  try{
    ...
    if(report.character.provenance.reviewRequired){
      pendingDdbImport=report;confirmedDdbSourceId=null;await saveJsonSetting(PENDING_DDB_SETTING,report);renderPendingSetupAccess();
      const message=`A newer ${report.character.name} sheet was found, but one or more definitions need 2024 review. Your saved character was not replaced.`;
      setCharacterRefreshStatus(message);if(manual){renderDdbReview(report);openImportCenter(`${message} Review the highlighted entries, then choose Import Reviewed Character when ready.`,'ddb');}return false;
    }
...
function renderInstalledPacks(){
  const root=$('#installed-pack-list');clear(root);$('#installed-pack-count').textContent=invalidPackRecords.length?`${installedPacks.length} ready · ${invalidPackRecords.length} needs review`:`${installedPacks.length} installed`;
  if(installedPacks.length===0&&invalidPackRecords.length===0){root.append(text('div','No private packs are installed on this device.','empty'));return;}
  ...
  for(const record of invalidPackRecords){
    const row=document.createElement('div');row.className='installed-pack invalid-pack';
    const copy=document.createElement('div');copy.append(text('strong',`Saved pack needs review: ${record.id}`),text('span','Altered kept this item instead of deleting it.'),text('small',record.reason));
    const actions=document.createElement('div');actions.className='pack-actions';
    actions.append(button('Export Recovery',()=>downloadJson(record.pack,`${slug(record.id)}-recovery.json`),'button compact'));
    actions.append(button('Remove',()=>{void (async()=>{if(!window.confirm(`Remove the unreadable saved pack “${record.id}”? Export it first if you may need to recover it.`))return;await removeExtensionPack(record.id);installedPacks=await loadValidatedInstalledPacks();renderInstalledPacks();renderSettings();setImportStatus(`${record.id} removed after confirmation.`);})();},'button compact danger'));
    row.append(copy,actions);root.append(row);
  }
}
...
async function loadValidatedInstalledPacks(){
  invalidPackCount=0;invalidPackRecords=[];const valid:OwnedContentPack[]=[];
  for(const record of await listExtensionPackRecords()){
    try{valid.push(parseOwnedContentPack(record.pack));}
    catch(error){invalidPackCount++;invalidPackRecords.push({id:record.id,pack:record.pack,reason:error instanceof Error?error.message:'This saved pack does not match the current schema.'});}
  }
  return valid;
}
...
function renderTransformSelector(){
  ...
  const deleteCustom=$<HTMLButtonElement>('#delete-custom-form');const owningPack=selected?.grantId?installedPacks.find(pack=>pack.content.transformationGrants.some(grant=>grant.id===selected.grantId)):undefined;deleteCustom.hidden=!owningPack;deleteCustom.disabled=Boolean(active&&selected?.id===active.id);deleteCustom.textContent=deleteCustom.disabled?'End Enhancement Before Deleting':`Delete ${selected?.label??'Custom Form'}`;deleteCustom.dataset.packId=owningPack?.metadata.id??'';deleteCustom.dataset.formLabel=selected?.label??'';
}
...
function hasExecutableBonusAction(){
  if(state.turn.bonusRemaining<=0)return false;
  const actionReady=sheet.actions.some(action=>action.cost==='bonus'&&!actionExecutionError(character,state,action,sheet.conditionImmunities));
  const spellReady=sheet.spells.some(spell=>spell.castingTime==='bonus'&&spell.available);
  const rageReady=classLevel(character,'Barbarian')>=1&&!state.rage.active&&!rageStartError(character,state);
  const rageExtensionReady=state.rage.active&&classLevel(character,'Barbarian')<15&&!actionCostError(state,'bonus',sheet.conditionImmunities);
  const secondWindReady=classLevel(character,'Fighter')>=1&&!secondWindError(character,state);
  const layOnHandsReady=classLevel(character,'Paladin')>=1&&!layOnHandsError(character,state,1);
  const configuredPrivateReady=sheet.features.some(feature=>feature.id.startsWith('private-')&&feature.activation==='bonus'&&feature.status!=='inactive'&&!actionCostError(state,'bonus',sheet.conditionImmunities));
  const active=state.activeTransform?.option;
  const formReady=availableTransformations(character,state).some(option=>option.profile!=='base'&&option.id!==active?.id&&option.actionCost==='bonus'&&option.usable&&!actionCostError(state,'bonus',sheet.conditionImmunities));
  const formExitReady=Boolean(active&&['wildshape','animal-shapes'].includes(active.profile));
  return actionReady||spellReady||rageReady||rageExtensionReady||secondWindReady||layOnHandsReady||configuredPrivateReady||formReady||formExitReady;
}
function turnReadyToEnd(){
  const bonusComplete=state.turn.bonusRemaining===0||!hasExecutableBonusAction();
  return !state.life.dead&&state.turn.actionsRemaining===0&&state.turn.surgeActionsRemaining===0&&bonusComplete&&(state.turn.attackAction?.remaining??0)===0&&pendingDamageRolls.size===0&&!state.pendingRelentlessRage&&!(state.hp===0&&!state.life.stable)&&state.concentrationChecks.length===0;
}
function syncTurnCompletionCue(){
  const ready=turnReadyToEnd();
  const explanation='The Action, Surge Action, Extra Attack sequence, pending damage, and required resolutions are complete, and no executable Bonus Action remains. End the turn when movement and any free interaction are finished; your Reaction can still be used off-turn.';
...
function passiveSkillScore(name:'Perception'|'Investigation'|'Insight'){
  const skill=sheet.skills[name];if(!skill)return {score:10,label:name,detail:'No imported skill modifier; showing 10.'};
  const mode=resolveAdvantage({advantage:skill.advantageSources??[],disadvantage:skill.disadvantageSources??[]}).mode;const situational=mode==='advantage'?5:mode==='disadvantage'?-5:0;
  return {score:10+skill.modifier+situational,label:name,detail:`10 ${signed(skill.modifier)}${situational?signed(situational):''} · ${skill.source}`};
}
function compactCoverageNames(names:Iterable<string>,empty:string){const unique=[...new Set([...names].map(name=>name.trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b));return {count:unique.length,copy:unique.length?`${unique.slice(0,4).join(', ')}${unique.length>4?` +${unique.length-4} more`:''}`:empty};}
function importedSummonNames(){
  const candidates=[...baseCharacter.feats.map(name=>({name,search:name})),...baseCharacter.features.map(feature=>({name:feature.name,search:`${feature.name} ${feature.summary}`})),...baseCharacter.spells.map(spell=>({name:spell.name,search:`${spell.name} ${spell.summary??''}`}))];
  return [...new Set(candidates.filter(entry=>/\b(familiar|summon(?:ed|ing|s)?|companion|homunculus|steed)\b/i.test(entry.search)).map(entry=>entry.name))];
}
function renderImportCoverage(){
  const unsupportedNames=new Set(baseCharacter.features.filter(feature=>feature.automation==='unsupported').map(feature=>feature.name));
  const structuredSpell=(spell:Spell)=>Boolean(spell.activeEffect||spell.attackBonus!==undefined||spell.healing||(spell.damage?.length&&spell.resolution!=='manual'));
  const executable=compactCoverageNames([
    ...sheet.features.filter(feature=>feature.status==='active').map(feature=>feature.name),
    ...baseCharacter.spells.filter(structuredSpell).map(spell=>spell.name),
    ...baseCharacter.items.filter(item=>Boolean(item.attack)&&item.mechanics!=='review-required').map(item=>item.name),
  ],'No structured imported feature mechanics detected.');
  const tracked=compactCoverageNames([
    ...sheet.features.filter(feature=>feature.status==='conditional'||feature.status==='inactive').map(feature=>feature.name),
    ...baseCharacter.spells.filter(spell=>!structuredSpell(spell)).map(spell=>spell.name),
  ],'No conditional mechanics need confirmation.');
  const reference=compactCoverageNames([
    ...sheet.features.filter(feature=>feature.status==='ruling'&&!unsupportedNames.has(feature.name)).map(feature=>feature.name),
    ...baseCharacter.items.filter(item=>item.mechanics==='reference-only').map(item=>item.name),
  ],'No reference-only imported features detected.');
  const pendingSetup=pendingDdbImport?.character.id===baseCharacter.id?pendingDdbImport.setupNeeds.map(need=>need.label):[];
  const review=compactCoverageNames([
    ...baseCharacter.features.filter(feature=>feature.automation==='unsupported').map(feature=>feature.name),
    ...baseCharacter.items.filter(item=>item.mechanics==='review-required').map(item=>item.name),
    ...pendingSetup,
    ...(baseCharacter.provenance.reviewRequired?['Character source validation']:[]),
  ],'Nothing is currently flagged for import review.');
  $('#import-coverage-summary').textContent=`${executable.count} calculated/executable · ${tracked.count} tracked/confirm · ${reference.count} reference · ${review.count} review`;
  const list=$('#import-coverage-list');clear(list);for(const entry of [
    {status:'success' as const,label:'Calculated / executable',value:executable},
    {status:'warning' as const,label:'Tracked / confirm',value:tracked},
    {status:'reference' as const,label:'Reference only',value:reference},
    {status:'warning' as const,label:'Needs review',value:review},
  ]){const row=document.createElement('div');row.className='import-coverage-row';row.append(statusChip(entry.status,`${entry.label} · ${entry.value.count}`),text('small',entry.value.copy));list.append(row);}
  const summonNames=importedSummonNames();const summonNote=$('#import-summons-note');summonNote.hidden=summonNames.length===0;if(summonNames.length)summonNote.textContent=`Not yet automated: ${summonNames.slice(0,3).join(', ')}${summonNames.length>3?' and other imported summons':''}. Altered preserves these as references but does not yet track separate AC, HP, statistics, or initiative for familiars, companions, and summons.`;
}
function renderCharacterSnapshot(){
  const abilities=$('#character-ability-scores');clear(abilities);for(const ability of ['str','dex','con','int','wis','cha'] as Ability[]){const score=sheet.abilities[ability],cell=document.createElement('span');cell.className='character-ability-score';cell.setAttribute('aria-label',`${ABILITY_NAMES[ability]} ${score}, modifier ${signed(abilityModifier(score))}`);cell.append(text('small',ability.toUpperCase()),text('b',String(score)),text('em',signed(abilityModifier(score))));abilities.append(cell);}
  const passives=$('#character-passive-scores');clear(passives);for(const passive of [passiveSkillScore('Perception'),passiveSkillScore('Investigation'),passiveSkillScore('Insight')]){const cell=document.createElement('span');cell.className='character-passive-score';cell.title=passive.detail;cell.append(text('small',passive.label),text('b',String(passive.score)));passives.append(cell);}
  $('#character-snapshot-context').textContent=sheet.form?`Current ${sheet.form.name} scores; retained mental abilities included`:'Current base character scores';renderImportCoverage();
}
function adjustTrackedResource(id:string,delta:-1|1){
  const pool=state.resources[id];if(!pool)return;const next=Math.max(0,Math.min(pool.max,pool.current+delta));if(next===pool.current)return;pool.current=next;notify(`${pool.name}: ${next}/${pool.max}. ${delta<0?'Marked one use manually.':'Restored one use manually.'}`);render();
}
function renderResources(){
  const strip=$('#resource-strip');clear(strip);
  const pools=Object.values(state.resources);if(pools.length){const note=text('p','Altered actions spend these automatically. Use Mark used only when you resolve a feature away from its Altered action.','resource-control-help');strip.append(note);}
  const recoveryLabels:Record<(typeof pools)[number]['recovery'],string>={'short-one':'Short Rest: restore one','short-all':'Short Rest: restore all','long-all':'Long Rest: restore all',manual:'Manual tracking'};
  for(const pool of pools){const row=document.createElement('article');row.className='tracked-resource';const copy=document.createElement('div');const status=pool.max===1?(pool.current?'Ready':'Used'):`${pool.current} of ${pool.max} left`;copy.append(text('strong',pool.name),text('small',`${status} · ${recoveryLabels[pool.recovery]}`));const controls=document.createElement('div');controls.className='tracked-resource-controls';const use=button('Mark used',()=>adjustTrackedResource(pool.id,-1),'button compact secondary');use.disabled=pool.current===0;use.setAttribute('aria-label',`Mark one ${pool.name} use spent; ${pool.current} of ${pool.max} remain`);const restore=button('Restore',()=>adjustTrackedResource(pool.id,1),'button compact secondary');restore.disabled=pool.current>=pool.max;restore.setAttribute('aria-label',`Restore one ${pool.name} use; ${pool.current} of ${pool.max} remain`);controls.append(use,restore);row.append(copy,text('b',`${pool.current}/${pool.max}`,'tracked-resource-count'),controls);strip.append(row);}
  const slotEntries=Object.entries(state.spellSlots).filter(([,slot])=>slot.max>0);if(slotEntries.length){const slots=document.createElement('div');slots.className='spell-slot-strip';slots.append(text('strong','Spell slots'));for(const [level,slot] of slotEntries){const node=document.createElement('span');node.className='resource-chip';node.append(document.createTextNode(`L${level} `),text('b',`${slot.current}/${slot.max}`));slots.append(node);}strip.append(slots);}
...
function receivedRollBonus(kind:ReceivedRollKind,skill?:string){
  ...
  for(const effect of state.receivedEffects){
    const quickGuidance=effect.kind==='guidance'&&kind==='skill'&&Boolean(skill);
    const eligible=effect.kind==='bless'&&(kind==='attack'||kind==='save')||quickGuidance;
    if(!eligible)continue;const die=rollDice('1d4').total;total+=die;applied.push(`${effect.name} +${die} [1d4]`);
    if(quickGuidance&&skill){state.receivedEffects=state.receivedEffects.filter(active=>active.id!==effect.id);applied.push(`Guidance chose ${skill} and ended after this check`);queueMicrotask(()=>{renderQuickReceivedEffects();renderConditions();persist();});}
  }
...
function receivedEffectSummary(effect:ReceivedEffect){const info=RECEIVED_EFFECTS[effect.kind],turns=finiteEffectTurns(effect.duration),elapsed=Math.max(0,state.turn.number-effect.addedTurn),remaining=turns===undefined?undefined:Math.max(0,turns-elapsed);return [info.summary,effect.kind==='guidance'?'Waiting for your next skill check; that skill is chosen automatically.':'',effect.kind==='heroic-inspiration'?'After an eligible d20 roll, Altered asks before rerolling. It is never spent automatically.':'',effect.kind!=='guidance'&&effect.skill?`Chosen skill: ${effect.skill}.`:'',effect.die?`Die: d${effect.die}.`:'',remaining!==undefined?`${remaining} turn${remaining===1?'':'s'} remaining.`:`Duration: ${effect.duration}.`].filter(Boolean).join(' ');}
...
function quickInspiration(){
  ...
  const heroic=state.receivedEffects.find(effect=>effect.kind==='heroic-inspiration');if(heroic){notify('Heroic Inspiration is ready. Altered will offer it after an eligible d20 roll and will not spend it without confirmation.');render();return;}
...
function renderQuickReceivedEffects(){
  ...
  const guidanceButton=$<HTMLButtonElement>('#quick-guidance');guidanceButton.classList.toggle('tracked-active',Boolean(guidance));guidanceButton.textContent=guidance?'Guidance · Next skill':'Guidance';guidanceButton.setAttribute('aria-pressed',String(Boolean(guidance)));guidanceButton.setAttribute('aria-label',guidance?'End Guidance waiting for the next skill check':'Add Guidance for the next skill check');
  const inspirationButton=$<HTMLButtonElement>('#quick-inspiration');const inspiration=bardic??heroic;inspirationButton.classList.toggle('tracked-active',Boolean(inspiration));inspirationButton.textContent=bardic?`Use Inspiration d${bardic.die??6}`:heroic?'Inspiration · Ready':'Inspiration';inspirationButton.setAttribute('aria-pressed',String(Boolean(inspiration)));inspirationButton.setAttribute('aria-label',bardic?`Roll and use Bardic Inspiration d${bardic.die??6}`:heroic?'Heroic Inspiration ready; use it only when Altered offers a reroll after a d20 roll':'Add and track Heroic Inspiration');
}
function renderActiveEffects(){
  ...
  if(state.rage.active){
    ...
    panel.append(text('strong','Rage is active'),text('span','B/P/S resistance · Advantage on Strength checks and saves','effect-benefit'),text('small','Spells and Concentration are blocked. Altered’s table rule treats Beast-form physical attacks as Unarmed Strikes, so eligible Strength-based beast attacks receive Rage Damage.'));
    cards.push(panel);
  }
  ...
  if(state.turn.oncePerTurn['action:dodge']){const panel=document.createElement('article');panel.className='effect-card tracked-active';panel.append(text('strong','Dodge active'),text('span','Until your next turn, qualifying attacks against you have Disadvantage and you have Advantage on Dexterity saving throws.','effect-benefit'));cards.push(panel);}
  if(state.turn.oncePerTurn['action:double-jump']){const panel=document.createElement('article');panel.className='effect-card tracked-active';panel.append(text('strong','Step of the Wind active'),text('span',`Jump distance is doubled for this turn.${state.turn.oncePerTurn['action:carry-ally']?' You may move one willing Large-or-smaller creature within 5 feet with you.':''}`,'effect-benefit'));cards.push(panel);}
  ...
  for(const effect of state.receivedEffects){
    ...
    if(effect.kind==='bardic-inspiration')panel.append(button(`Failed a D20 Test? Roll d${effect.die??6} & use`,()=>consumeReceivedEffect(effect),'button compact primary'));
    if(effect.kind==='heroic-inspiration'&&pendingHeroicRoll)appendHeroicRerollControls(panel,effect,'button compact primary');
    panel.append(button(`End ${effect.name}`,()=>{if(effect.kind==='heroic-inspiration')pendingHeroicRoll=undefined;applyResult(endReceivedEffect(state,effect.id));},'button compact secondary'));cards.push(panel);
  }
...
function renderQuickFeatures(){
  ...
  if(classLevel(character,'Barbarian')>=2){const error=recklessAttackError(character,state);const reckless=button(state.rage.recklessDeclared?'Reckless On':'Reckless',()=>applyResult(declareRecklessAttack(character,state)),'button secondary'+(state.rage.recklessDeclared?' active':''));reckless.disabled=Boolean(error);if(error)reckless.title=error;box.append(reckless);}
  if(classLevel(character,'Fighter')>=1){const error=secondWindError(character,state);const secondWind=button('Second Wind',()=>{const roll=rollDice('1d10').total;applyResult(useSecondWind(character,state,roll));});secondWind.disabled=Boolean(error);if(error)secondWind.title=error;box.append(secondWind);}
  if(classLevel(character,'Fighter')>=2){const error=actionSurgeError(character,state);const surge=button('Action Surge',()=>applyResult(useActionSurge(character,state)));surge.disabled=Boolean(error);if(error)surge.title=error;box.append(surge);}
  if(classLevel(character,'Paladin')>=1){const amount=Number($<HTMLInputElement>('#damage-amount').value);const error=layOnHandsError(character,state,amount);const layOnHands=button('Lay On Hands',()=>applyResult(useLayOnHands(character,state,Number($<HTMLInputElement>('#damage-amount').value))));layOnHands.disabled=Boolean(error);if(error)layOnHands.title=error;box.append(layOnHands);}
...
function rollD20Result(mod:number,mode:RollMode,criticalThreshold=20):D20Result{
  const result=rollAttackD20(mod,mode,criticalThreshold);latestD20Context={result,modifier:mod,criticalThreshold};return result;
}
...
function dismissRollToast(){const toast=$('#roll-toast');if(rollToastTimer!==undefined){window.clearTimeout(rollToastTimer);rollToastTimer=undefined;}toast.classList.remove('show');toast.hidden=true;}
function resolveHeroicReroll(effect:ReceivedEffect,dieIndex:0|1){
  const pending=pendingHeroicRoll;if(!pending||effect.kind!=='heroic-inspiration'||!state.receivedEffects.some(active=>active.id===effect.id))return;
  const replacement=rollDice('1d20').total;let first=pending.result.first,second=pending.result.second;const replaced=dieIndex===1&&second!==undefined?second:first;if(dieIndex===1&&second!==undefined)second=replacement;else first=replacement;
  const kept=second===undefined?first:pending.result.mode==='advantage'?Math.max(first,second):Math.min(first,second);const total=kept+pending.modifier+pending.displayAdjustment;const result:D20Result={first,...(second!==undefined?{second}:{}),kept,total,mode:pending.result.mode,critical:kept>=pending.criticalThreshold,naturalOne:kept===1,naturalTwenty:kept===20};const title=pending.title;
  state.receivedEffects=state.receivedEffects.filter(active=>active.id!==effect.id);pendingHeroicRoll=undefined;latestD20Context=undefined;
  showRoll(total,`${modeText(result)}. Heroic Inspiration replaced ${replaced} with ${replacement}; the new roll was used and Inspiration was consumed.`,`${title} · Heroic Inspiration`,d20Presentation(kept,total));render();
}
function appendHeroicRerollControls(parent:HTMLElement,effect:ReceivedEffect,className='button compact primary'){
  const pending=pendingHeroicRoll;if(!pending)return;const controls=document.createElement('span');controls.className='heroic-reroll-controls';controls.append(text('small','Spend Heroic Inspiration? The replacement die must be used.'));
  const addChoice=(dieIndex:0|1,value:number)=>{const control=button(`Reroll ${pending.result.second===undefined?'d20':`die ${dieIndex+1} (${value})`}`,()=>resolveHeroicReroll(effect,dieIndex),className);control.addEventListener('click',event=>event.stopPropagation());controls.append(control);};addChoice(0,pending.result.first);if(pending.result.second!==undefined)addChoice(1,pending.result.second);parent.append(controls);
}
function renderToastHeroicControls(){const root=$('#roll-toast-inspiration');clear(root);const effect=state.receivedEffects.find(entry=>entry.kind==='heroic-inspiration');root.hidden=!effect||!pendingHeroicRoll;if(effect&&pendingHeroicRoll)appendHeroicRerollControls(root,effect,'button compact primary');}
function showFloatingRoll(totalText:string,detail:string,title:string,presentation:RollPresentation={}){
  ...
  renderToastHeroicControls();toast.hidden=false;void toast.offsetWidth;toast.classList.add('show');
  rollToastTimer=window.setTimeout(dismissRollToast,8000);
}
function showRoll(total:number|string,detail:string,title='Roll result',presentation:RollPresentation={}){latestRollTab=currentTab;latestRollTitle=title;const panel=$('#latest-roll');const totalText=String(total),context=latestD20Context;latestD20Context=undefined;const heroic=state.receivedEffects.some(effect=>effect.kind==='heroic-inspiration');if(heroic&&presentation.natural!==undefined&&context){const displayed=totalText.match(/^-?\d+/)?.[0],numeric=displayed===undefined?context.result.total:Number(displayed);pendingHeroicRoll={...context,displayAdjustment:numeric-context.result.total,title};}else pendingHeroicRoll=undefined;$('#roll-title').textContent=title;$('#roll-total').textContent=totalText;$('#roll-detail').textContent=detail;$('#play-roll-title').textContent=title;$('#play-roll-total').textContent=totalText;$('#play-roll-detail').textContent=detail;panel.classList.remove('flash');void panel.offsetWidth;panel.classList.add('flash');showFloatingRoll(totalText,detail,title,presentation);addActivity(`${title}: ${detail}`);renderLog();}
...
function attackMinimum(action:CreatureAction){if(action.type!=='attack')return 0;const kinds:('weapon'|'unarmed')[]=action.kind==='weapon'?['weapon']:attackCountsAsUnarmedStrike(character,state,action)?['unarmed']:[];return Math.max(0,...sheet.attackDamageModifiers.filter(modifier=>kinds.some(kind=>modifier.appliesTo.includes(kind))).map(modifier=>modifier.minimumDamage??0));}
...
function optionalDamageChoice(actionId:string,group:string,bonuses:DamagePacket[]){const label=document.createElement('label');label.className='slot-picker';label.append(text('span',group==='primal-strike'?'Primal Strike damage':'Optional damage'));const select=document.createElement('select');select.setAttribute('aria-label',group==='primal-strike'?'Choose one Primal Strike damage type':'Choose one optional damage type');const none=document.createElement('option');none.value='';none.textContent='Do not use';select.append(none);const selected=selectedOptionalDamagePackets(bonuses,optionalSet(actionId))[0]?.label??'';for(const bonus of bonuses){const option=document.createElement('option');option.value=bonus.label??'';option.textContent=`${bonus.type} (${bonus.expression})`;option.selected=option.value===selected;select.append(option);}select.addEventListener('change',()=>{const set=optionalSet(actionId);for(const bonus of bonuses)if(bonus.label)set.delete(bonus.label);if(select.value)set.add(select.value);});label.append(select);return label;}
...
function resolveAttackAction(action:AttackAction){
  const key=actionPrerequisiteKey(action);if(action.prerequisite&&!confirmedActionPrerequisites.has(key)){notify('Confirm the visible trigger before using this Reaction.');return;}
  ...
  const radiant=radiantActions.has(action.id),bonuses=attackBonuses(character,state,sheet,action);const automatic=bonuses.filter(p=>!p.label?.startsWith('Optional'));
  const selected=optionalSet(action.id);const optional=selectedOptionalDamagePackets(bonuses.filter(p=>p.label?.startsWith('Optional')),selected);
...
function resolveAutomaticAction(action:Extract<CreatureAction,{type:'automatic'}>,choice?:AutomaticActionChoice){
  ...
  const choicePool=choice?.resourceId?state.resources[choice.resourceId]:undefined,choiceCost=choice?.resourceCost??1;if(choice?.resourceId&&(!choicePool||choicePool.current<choiceCost)){notify(`Needs ${choiceCost} ${choicePool?.name??choice.resourceId}; ${choicePool?.current??0} remain.`);render();return;}
  if(!prepareLimitedAction(action))return;
  if(choicePool)choicePool.current-=choiceCost;
  const grants=new Set(choice?.grants??[]);if(choice?.resolution==='dash')grants.add('dash');if(choice?.resolution==='disengage')grants.add('disengage');for(const grant of grants)state.turn.oncePerTurn[`action:${grant}`]=true;
  if(action.id==='monk-patient-defense'&&choice?.id==='focused-defense'&&classLevel(character,'Monk')>=10){const die=Number(monkDie(classLevel(character,'Monk')).slice(2)),first=1+Math.floor(Math.random()*die),second=1+Math.floor(Math.random()*die),incoming=first+second;if(incoming>state.tempHp){state.tempHp=incoming;state.tempHpSource='Heightened Focus · Patient Defense';}notify(`Patient Defense granted ${incoming} Temporary Hit Points (${first} + ${second}).`);}
...
function resolveMultiattack(action:Extract<CreatureAction,{type:'multiattack'}>){
  ...
  for(const id of multiattackSequence(action)){
    ...
    if(child.type==='attack'){
      ...
      const bonuses=attackBonuses(character,state,sheet,child),automatic=bonuses.filter(packet=>!packet.label?.startsWith('Optional')),selected=optionalSet(child.id),optional=selectedOptionalDamagePackets(bonuses.filter(packet=>packet.label?.startsWith('Optional')),selected),riders=activeRiders(child),radiant=radiantActions.has(child.id);
...
function renderActions(scope:'all'|'reactions'='all'){
  const root=$('#tab-content');clear(root);const actions=scope==='reactions'?sheet.actions.filter(action=>action.cost==='reaction'):sheet.actions;root.append(text('p',scope==='reactions'?'Use a Reaction only after its trigger occurs. Executable controls below spend the Reaction through the same rules path as Actions and Spells; reference cards never spend it.':'Choose an action below. A used Action, Bonus Action, or Reaction is disabled until the next turn so it is always clear what is still available.','tab-help'));
  if(scope==='all'&&pendingDamageRolls.size){const pending=document.createElement('section');pending.className='pending-damage-list';pending.append(text('h3',`Resolve confirmed hits (${pendingDamageRolls.size})`));for(const roll of pendingDamageRolls.values()){const row=document.createElement('article');row.className='pending-damage-card tracked-active';const copy=document.createElement('div');copy.append(text('strong',roll.name),text('small',`${roll.critical?'Critical hit · ':''}${roll.packets.map(packet=>`${roll.critical&&packet.doubleOnCritical!==false?criticalDiceExpression(packet.expression):packet.expression} ${packet.type}`).join(' + ')}. Roll only if the attack hit.`));const actions=document.createElement('div');actions.className='inline-actions';actions.append(button('Roll Damage',()=>resolvePendingDamage(roll.id),'button primary action-roll'),button('Miss · Clear',()=>{pendingDamageRolls.delete(roll.id);notify(`${roll.name} missed; pending damage cleared.`);render();},'button secondary compact'));row.append(copy,actions);pending.append(row);}root.append(pending);}
  if(actions.length===0){if(scope==='all')root.append(text('div','No actions are defined for this form.','empty'));return;}if(scope==='reactions')root.append(text('h3',`Executable Reaction actions (${actions.length})`));
  ...
  for(const action of actions){
    ...
    if(action.type==='attack'){
      const limit=limitedActionStatus(action);const blocked=limit.unavailable||Boolean(economyError),key=actionPrerequisiteKey(action),confirmed=!action.prerequisite||confirmedActionPrerequisites.has(key);const limitText=actionLimitText(action);const description=[action.prerequisite?`Trigger: ${action.prerequisite}`:'',`${signed(action.attackBonus)} to hit`,action.damage.map(packet=>`${packet.expression} ${packet.type}`).join(' + '),action.effects?.length?`On hit: ${effectsText(action.effects)}`:'',sequenceText,limitText,action.notes,economyError].filter(Boolean).join(' · ');const c=card(action.name,limit.recharge?'Recharging':limit.remaining===0?'Expended':action.prerequisite&&!confirmed?'Confirm trigger':sequenceText||actionCostLabel(action.cost),description,blocked?(limit.unavailable?'locked':'unavailable'):action.prerequisite&&!confirmed?'warning':'available');if(blocked)c.badge.className='badge inactive';
      if(action.prerequisite)c.options.append(toggleRow(`Confirm trigger: ${action.prerequisite}`,confirmed,checked=>{checked?confirmedActionPrerequisites.add(key):confirmedActionPrerequisites.delete(key);renderTab();renderNextStepGuide();}));
      ...
      const optionalBonuses=attackBonuses(character,state,sheet,action).filter(packet=>packet.label?.startsWith('Optional')),renderedChoiceGroups=new Set<string>();for(const bonus of optionalBonuses){const label=bonus.label??'Optional damage';if(bonus.choiceGroup){if(renderedChoiceGroups.has(bonus.choiceGroup))continue;renderedChoiceGroups.add(bonus.choiceGroup);c.options.append(optionalDamageChoice(action.id,bonus.choiceGroup,optionalBonuses.filter(candidate=>candidate.choiceGroup===bonus.choiceGroup)));continue;}c.options.append(toggleRow(`${label} (${bonus.expression} ${bonus.type})`,optionalSet(action.id).has(label),checked=>{const set=optionalSet(action.id);checked?set.add(label):set.delete(label);}));}
      ...
      collapseActionOptions(c.options);const label=limit.recharge?`Awaiting ${limit.recharge.min}–${limit.recharge.max}`:limit.remaining===0?'No uses remaining':economyError??(!confirmed?'Confirm the trigger above':`Roll ${action.name} Attack`);const use=button(label,()=>resolveAttackAction(action),blocked||!confirmed?'button secondary action-roll':'button primary action-roll');use.disabled=blocked||!confirmed;c.actions.append(use);appendAction(c.node,blocked);
    }else if(action.type==='save'){
      const limit=limitedActionStatus(action);const blocked=limit.unavailable||Boolean(economyError),key=actionPrerequisiteKey(action),confirmed=!action.prerequisite||confirmedActionPrerequisites.has(key);const damage=action.damageOnFail?.map(packet=>`${packet.expression} ${packet.type}`).join(' + ')||'Effect on failed save';const detail=[action.prerequisite?`Trigger: ${action.prerequisite}`:'',damage,action.halfOnSuccess?'Half damage on a successful save (rounded down)':'',action.effectsOnFail?.length?`On failure: ${effectsText(action.effectsOnFail)}`:'',sequenceText,actionLimitText(action),action.notes,economyError].filter(Boolean).join(' · ');const c=card(action.name,limit.recharge?'Recharging':limit.remaining===0?'Expended':action.prerequisite&&!confirmed?'Confirm trigger':`${sequenceText||actionCostLabel(action.cost)} · DC ${action.dc} ${saveAbilities(action)}`,detail,blocked?(limit.unavailable?'locked':'unavailable'):action.prerequisite&&!confirmed?'warning':'available');if(blocked)c.badge.className='badge inactive';if(action.prerequisite)c.options.append(toggleRow(`Confirm prerequisite: ${action.prerequisite}`,confirmed,checked=>{checked?confirmedActionPrerequisites.add(key):confirmedActionPrerequisites.delete(key);renderTab();renderNextStepGuide();}));collapseActionOptions(c.options);const label=limit.recharge?`Awaiting ${limit.recharge.min}–${limit.recharge.max}`:limit.remaining===0?'No uses remaining':economyError??(!confirmed?'Confirm the hit or trigger above':`Use ${action.name}`);const use=button(label,()=>resolveSaveAction(action),blocked||!confirmed?'button secondary action-roll':'button primary action-roll');use.disabled=blocked||!confirmed;if(!confirmed)use.title='Confirm the visible prerequisite first so no resource is spent early.';c.actions.append(use);appendAction(c.node,blocked);
      ...
      if(action.choices?.length){for(const choice of action.choices){const key=actionPrerequisiteKey(action,choice),confirmed=!choice.prerequisite||confirmedActionPrerequisites.has(key),pool=choice.resourceId?state.resources[choice.resourceId]:undefined,resourceCost=choice.resourceCost??1,resourceError=choice.resourceId&&(!pool||pool.current<resourceCost)?`Needs ${resourceCost} ${pool?.name??choice.resourceId}; ${pool?.current??0} remain.`:'';if(choice.prerequisite)c.options.append(toggleRow(`Confirm ${choice.label}: ${choice.prerequisite}`,confirmed,checked=>{checked?confirmedActionPrerequisites.add(key):confirmedActionPrerequisites.delete(key);renderTab();renderNextStepGuide();}));const optionBlocked=blocked||Boolean(resourceError),label=optionBlocked?(resourceError||(limit.recharge?`Awaiting ${limit.recharge.min}–${limit.recharge.max}`:limit.remaining===0?'No uses remaining':economyError??'Unavailable')):`Use ${action.name}: ${choice.label}`;const use=button(label,()=>resolveAutomaticAction(action,choice),optionBlocked||!confirmed?'button secondary action-roll':'button primary action-roll');use.disabled=optionBlocked||!confirmed;const reason=resourceError||(!confirmed?`Confirm the ${choice.label} prerequisite above.`:'');if(reason){use.title=reason;const explanation=document.createElement('small');explanation.className='control-reason';explanation.textContent=reason;c.actions.append(explanation);}c.actions.append(use);}collapseActionOptions(c.options);}
      else{const key=actionPrerequisiteKey(action),confirmed=!action.prerequisite||confirmedActionPrerequisites.has(key);if(action.prerequisite)c.options.append(toggleRow(`Confirm prerequisite: ${action.prerequisite}`,confirmed,checked=>{checked?confirmedActionPrerequisites.add(key):confirmedActionPrerequisites.delete(key);renderTab();renderNextStepGuide();}));collapseActionOptions(c.options);const label=limit.recharge?`Awaiting ${limit.recharge.min}–${limit.recharge.max}`:limit.remaining===0?'No uses remaining':economyError??`Use ${action.name}`;const use=button(label,()=>resolveAutomaticAction(action),blocked||!confirmed?'button secondary action-roll':'button primary action-roll');use.disabled=blocked||!confirmed;if(!confirmed)use.title='Confirm the visible prerequisite first.';c.actions.append(use);}appendAction(c.node,blocked);
    }
  }
  for(const multiattack of actions.filter((candidate):candidate is Extract<CreatureAction,{type:'multiattack'}>=>candidate.type==='multiattack')){
...
function comparableMechanicName(value:string){return value.toLowerCase().replace(/[^a-z0-9]+/g,'');}
function renderReactions(){
  renderActions('reactions');const root=$('#tab-content'),reactionActions=sheet.actions.filter(action=>action.cost==='reaction'),reactionSpells=sheet.spells.filter(spell=>spell.castingTime==='reaction');
  if(reactionSpells.length){root.append(text('h3',`Reaction spells (${reactionSpells.length})`));for(const spell of reactionSpells)root.append(spellCard(spell));}
  const actionNames=new Set(reactionActions.flatMap(action=>[comparableMechanicName(action.id),comparableMechanicName(action.name)]));const reactionFeatures=sheet.features.filter(feature=>feature.activation==='reaction');
  const unmatchedFeatures=reactionFeatures.filter(feature=>{const id=comparableMechanicName(feature.id),name=comparableMechanicName(feature.name);return ![...actionNames].some(actionName=>actionName.includes(id)||actionName.includes(name)||id.includes(actionName)||name.includes(actionName));});
  if(unmatchedFeatures.length){root.append(text('h3','Conditional and reference Reactions'));for(const feature of unmatchedFeatures){if(feature.id.startsWith('private-')&&feature.status!=='inactive'){root.append(featureCard(feature));continue;}const available=state.turn.reactionRemaining>0&&feature.status!=='inactive';const c=card(feature.name,'Conditional · reference',`${feature.summary} Trigger: ${feature.reason} The current engine has not exposed an executable control for this Reaction, so this card does not spend it; resolve it from the authorized source.`,available?'warning':'inactive');c.badge.className='badge conditional';root.append(c.node);}}
  const hasOpportunity=reactionActions.some(action=>/opportunity attack/i.test(action.name)||/opportunity[-_ ]?attack/i.test(action.id));if(!hasOpportunity){const reactionReady=state.turn.reactionRemaining>0&&!actionCostError(state,'reaction',sheet.conditionImmunities);const opportunity=card('Opportunity Attack','Conditional · reference',`${reactionReady?'Reaction ready.':'Reaction unavailable right now.'} Use only when a creature you can see leaves your reach using its action, Bonus Action, Reaction, or movement. The current engine has not exposed an executable Opportunity Attack for this sheet, so choose an eligible melee attack in Actions and resolve the trigger with your DM; this reference card does not spend the Reaction.`,reactionReady?'warning':'inactive');opportunity.badge.className='badge conditional';opportunity.actions.append(button('Choose eligible melee attack',()=>openTask('actions',taskOrigin),'button secondary'));root.append(opportunity.node);}
  const deflectFeature=sheet.features.find(feature=>/deflect attacks?/i.test(feature.name)||/deflect[-_ ]?attacks?/i.test(feature.id)),hasDeflectAction=reactionActions.some(action=>/deflect attacks?/i.test(action.name)||/deflect[-_ ]?attacks?/i.test(action.id));if(deflectFeature&&!hasDeflectAction&&!unmatchedFeatures.includes(deflectFeature)){const deflect=card(deflectFeature.name,'Conditional · reference',`${deflectFeature.summary} The imported feature is present, but no executable Deflect Attacks Reaction is exposed for this sheet yet. Resolve its trigger from the source; this card does not spend the Reaction.`,'warning');deflect.badge.className='badge conditional';root.append(deflect.node);}
  const executableCount=reactionActions.length+reactionSpells.filter(spell=>spell.available).length+unmatchedFeatures.filter(feature=>feature.id.startsWith('private-')&&feature.status!=='inactive').length;if(!executableCount)root.append(text('p','No executable Reaction is available for the current sheet. Conditional reference entries remain visible so missing automation is explicit.','empty reaction-empty'));
}
function renderRolls(){
  ...
  for(const value of Object.values(sheet.saves)){const row=document.createElement('div');row.className='roll-row';const info=document.createElement('div');const ruleMode=resolveAdvantage({advantage:value.advantageSources??[],disadvantage:value.disadvantageSources??[]}).mode as 'normal'|'advantage'|'disadvantage';const notes=[value.source,...(value.advantageSources?.length?[`Advantage: ${value.advantageSources.join(', ')}`]:[]),...(value.disadvantageSources?.length?[`Disadvantage: ${value.disadvantageSources.join(', ')}`]:[]),...(value.minimumTotal!==undefined?[`${value.minimumSource}: minimum total ${value.minimumTotal}`]:[]),...(state.receivedEffects.some(effect=>effect.kind==='bless')?['Bless adds 1d4 automatically.']:[]),...(value.automaticFailure?[value.automaticFailure]:[])];info.append(text('strong',`${value.name} ${signed(value.modifier)}`),text('small',notes.join(' · ')));const roll=button(value.automaticFailure?'Automatic Failure':'Roll',()=>d20(value.modifier,combinedMode(ruleMode),value.name,value.minimumD20,value.minimumTotal,value.minimumSource,undefined,'save'),'button compact');const saveLabel=/\bsave\b/i.test(value.name)?value.name:`${value.name} save`;roll.setAttribute('aria-label',value.automaticFailure?`${saveLabel} automatically fails`:`Roll ${saveLabel}`);roll.disabled=Boolean(value.automaticFailure);row.append(info,roll);saves.append(row);}root.append(saves,text('h3','Skills'));
  const skills=document.createElement('div');skills.className='roll-grid';for(const value of Object.values(sheet.skills).sort((a,b)=>a.name.localeCompare(b.name))){const row=document.createElement('div');row.className='roll-row';const info=document.createElement('div');const ruleMode=resolveAdvantage({advantage:value.advantageSources??[],disadvantage:value.disadvantageSources??[]}).mode as 'normal'|'advantage'|'disadvantage';const guided=state.receivedEffects.some(effect=>effect.kind==='guidance'&&effect.skill===value.name);const detail=[value.source,...(value.advantageSources?.length?[`Advantage: ${value.advantageSources.join(', ')}`]:[]),...(value.disadvantageSources?.length?[`Disadvantage: ${value.disadvantageSources.join(', ')}`]:[]),...(value.conditionalSources?.length?[`Conditional: ${value.conditionalSources.join(', ')}`]:[]),...(value.minimumD20!==undefined?[`${value.minimumSource}: d20 minimum ${value.minimumD20}`]:[]),...(value.minimumTotal!==undefined?[`${value.minimumSource}: total minimum ${value.minimumTotal}`]:[]),...(guided?['Guidance adds 1d4 automatically.']:[]),...(value.alternate?[`Alternate ${signed(value.alternate.modifier)}: ${value.alternate.source}`]:[])].join(' · ');info.append(text('strong',`${value.name} ${signed(value.modifier)}`),text('small',detail));const actions=document.createElement('div');actions.className='inline-actions';const skillRoll=button('Roll',()=>d20(value.modifier,combinedMode(ruleMode),value.name,value.minimumD20,value.minimumTotal,value.minimumSource,undefined,'skill',value.name),'button compact');skillRoll.setAttribute('aria-label',`Roll ${value.name} skill check`);actions.append(skillRoll);if(value.alternate){const alternateRoll=button('Use STR',()=>d20(value.alternate?.modifier??value.modifier,combinedMode(ruleMode),`${value.name} (Strength)`,value.alternate?.minimumD20,value.alternate?.minimumTotal,value.alternate?.minimumSource,undefined,'skill',value.name),'button compact');alternateRoll.setAttribute('aria-label',`Roll ${value.name} skill check using Strength`);actions.append(alternateRoll);}row.append(info,actions);skills.append(row);}root.append(skills);
}
...
function chosenSpellCastingOption(spell:Spell):SpellCastingOption|undefined{
  const minimum=spell.slotLevel??spell.level;if(minimum===0)return undefined;
  const options=availableSpellCastingOptions(character,state,minimum),selected=selectedSpellCastingOptions.get(spellKey(spell));
  return options.find(option=>option.id===selected)??options[0];
}
function chosenSpellLevel(spell:Spell){
  const minimum=spell.slotLevel??spell.level;if(minimum===0)return 0;return chosenSpellCastingOption(spell)?.level??minimum;
}
function chosenTransformationCastingOption(option:TransformationOption){
  if(!option.spellName)return undefined;const matches=baseCharacter.spells.filter(spell=>spell.name.trim().toLowerCase()===option.spellName?.trim().toLowerCase());
  const selected=matches.find(spell=>selectedSpellCastingOptions.has(spellKey(spell)))??matches[0];return selected?chosenSpellCastingOption(selected):undefined;
}
...
function castAndResolveSpell(spell:Spell&{available:boolean;reason:string}){const choice=chosenSpellCastingOption(spell),castLevel=choice?.level??(spell.slotLevel??spell.level);const result=castSpell(character,state,spell.name,choice?.level,choice?.resourceId);const success=result.message.startsWith('Cast ');applyResult(result);if(!success)return;const immediate=spell.resolution!=='manual'&&(spell.attackBonus!==undefined||Boolean(spell.healing)||(!spell.concentration&&Boolean(spell.damage?.length)));const persistent=spellActiveEffect(spell);const timing=persistent?.id==='barkskin'?' Barkskin used your Bonus Action; your Action remains available. Wild Shape and Rage also require a Bonus Action and must wait until a later turn.':'';if(immediate)resolveSpellEffect(spell,castLevel);else showRoll('Cast',`${persistent?`${persistent.summary} Active for ${persistent.duration}.`:spell.concentration?'Concentration started. Use Resolve Effect whenever the spell deals damage or healing.':spell.summary??'Spell activated.'}${timing}`,spell.name);}
const TRANSFORMATION_SPELL_PROFILES:Record<string,'polymorph'|'true-polymorph'|'shapechange'|'animal-shapes'>={polymorph:'polymorph','true polymorph':'true-polymorph',shapechange:'shapechange','animal shapes':'animal-shapes'};
function transformationSpellProfile(spellName:string){return TRANSFORMATION_SPELL_PROFILES[spellName.trim().toLowerCase()];}
function chooseTransformationSpellForm(spellName:string){
  const profile=transformationSpellProfile(spellName);if(!profile)return;
  formFilter='spell';formSearch=spellName;
  $<HTMLSelectElement>('#form-filter').value='spell';$<HTMLInputElement>('#form-search').value=spellName;
  const options=availableTransformations(character,state).filter(option=>option.profile===profile);
  if(!options.some(option=>option.id===selectedOptionId))selectedOptionId=options[0]?.id??'base';
  renderTransformSelector();renderArt();setWorkspace('forms',undefined,taskOrigin,$<HTMLElement>('#forms-view-title'));
}
function spellCard(spell:Spell&{available:boolean;reason:string}){
  ...
  const castingOptions=availableSpellCastingOptions(character,state,slotLevel),chosenCastingOption=chosenSpellCastingOption(spell);if(slotLevel>0&&castingOptions.length){const picker=document.createElement('label');picker.className='slot-picker';picker.append(text('span','Cast using'));const select=document.createElement('select');select.setAttribute('aria-label',`${spell.name} casting resource`);const scales=Boolean(spell.higherSlotHealing||spell.higherSlotDamage?.length);for(const castingOption of castingOptions){const option=document.createElement('option');option.value=castingOption.id;option.textContent=`${castingOption.label}${castingOption.level>slotLevel?scales?' · scaled effect':' · higher-level slot':''}`;option.selected=castingOption.id===chosenCastingOption?.id;select.append(option);}select.disabled=!spell.available;select.addEventListener('change',()=>{selectedSpellCastingOptions.set(spellKey(spell),select.value);renderSpells();renderNextStepGuide();});picker.append(select);c.options.append(picker);}if(spell.attackBonus!==undefined)c.options.append(rollModePicker(`spell-${spell.name}`,spell.name));
  const transformationProfile=transformationSpellProfile(spell.name);const immediate=spell.resolution!=='manual'&&(spell.attackBonus!==undefined||Boolean(spell.healing)||(!spell.concentration&&Boolean(spell.damage?.length)));const castLabel=transformationProfile?`Choose ${spell.name} Form`:persistent?.id==='barkskin'?'Cast Barkskin on Self':immediate?`Cast & Roll ${spell.name}`:`Cast ${spell.name}`;const cast=button(spell.available?castLabel:spell.reason,()=>transformationProfile?chooseTransformationSpellForm(spell.name):castAndResolveSpell(spell),spell.available?'button primary action-roll':'button secondary');cast.disabled=!spell.available;c.actions.append(cast);const activeEffect=state.concentration?.name===spell.name&&spell.resolution!=='manual'&&Boolean(spell.damage?.length||spell.healing);if(activeEffect)c.actions.append(button(`Resolve ${spell.name} Effect`,()=>resolveSpellEffect(spell,state.concentration?.castLevel??slotLevel),'button secondary'));return c.node;
}
...
function featureHowToUse(feature:EvaluatedFeature,ownedFeat:boolean){
  ...
  if(feature.id==='reckless-attack')return state.rage.recklessDeclared?'Declared now. Eligible Strength attacks gain Advantage; attacks against you gain Advantage until your next turn.':'Tap Declare Reckless before your first attack roll, or leave it off.';
...
function appendFeatureControls(feature:EvaluatedFeature,actions:HTMLElement,priority?:PriorityAbilityState){
  ...
  if(feature.id==='reckless-attack'){const error=recklessAttackError(character,state);const control=button(state.rage.recklessDeclared?'Cancel Reckless':'Declare Reckless',()=>applyResult(declareRecklessAttack(character,state)),state.rage.recklessDeclared?'button primary active':'button primary');control.disabled=Boolean(error);appendControlReason(actions,control,error,'reckless-attack-blocked-reason');return;}
  if(feature.id==='second-wind'){const error=secondWindError(character,state);const control=button('Use Second Wind · Bonus Action',()=>applyResult(useSecondWind(character,state,rollDice('1d10').total)),error?'button secondary':'button primary');control.disabled=Boolean(error);appendControlReason(actions,control,error,'second-wind-blocked-reason');return;}
  if(feature.id==='action-surge'){const error=actionSurgeError(character,state);const control=button(state.turn.surgeActionsRemaining>0?'Extra Action Available':'Use Action Surge',()=>applyResult(useActionSurge(character,state)),error?'button secondary':'button primary');control.disabled=Boolean(error)||state.turn.surgeActionsRemaining>0;appendControlReason(actions,control,error,'action-surge-blocked-reason');return;}
  if(feature.id==='lay-on-hands'){const amount=Math.max(1,Number($<HTMLInputElement>('#damage-amount').value)||1),error=layOnHandsError(character,state,amount);const control=button(`Use Lay On Hands · ${amount} point${amount===1?'':'s'}`,()=>applyResult(useLayOnHands(character,state,Number($<HTMLInputElement>('#damage-amount').value))),error?'button secondary':'button primary');control.disabled=Boolean(error);appendControlReason(actions,control,error,'lay-on-hands-blocked-reason');return;}
...
function renderTab(){sheet=resolveSheet(character,state);syncTabState();if(currentTab==='actions')renderActions();else if(currentTab==='reactions')renderReactions();else if(currentTab==='rolls')renderRolls();else if(currentTab==='spells')renderSpells();else if(currentTab==='features')renderFeatures();else if(currentTab==='equipment')renderEquipment();else renderRules();}
...
function renderConditions(){
  ...
  for(const effect of state.receivedEffects){const card=document.createElement('article');card.className='received-effect-entry tracked-active';const copy=document.createElement('div');copy.append(text('strong',effect.name),text('small',receivedEffectSummary(effect)));const actions=document.createElement('div');actions.className='received-effect-actions';if(effect.kind==='bardic-inspiration')actions.append(button(`Use d${effect.die??6}`,()=>consumeReceivedEffect(effect),'button compact primary'));if(effect.kind==='heroic-inspiration'&&pendingHeroicRoll)appendHeroicRerollControls(actions,effect,'button compact primary');actions.append(button('End',()=>{pendingHeroicRoll=undefined;applyResult(endReceivedEffect(state,effect.id));},'button compact secondary'));card.append(copy,actions);effects.append(card);}
}
...
function recommendNextStep():NextStepSuggestion|null{
  if(currentWorkspace==='forms'){
    ...
    if(!transform.hidden&&transform.disabled){
      ...
      if(needsTurn){
        if(pendingDamageRolls.size)return {title:'Resolve damage',copy:'An attack hit is waiting for its separate damage roll. Resolve it before ending this turn.',target:transform,reveal:revealTask('actions','Roll Damage')};
        if(state.turn.actionsRemaining>0||state.turn.surgeActionsRemaining>0||(state.turn.attackAction?.remaining??0)>0)return {title:'Use your remaining Action',copy:'This transformation is waiting for a new Bonus Action, but you can still use the Action available this turn.',target:transform,reveal:revealTask('actions')};
        if(turnReadyToEnd())return {title:'Finish this turn',copy:'The required action is spent and no other tracked action is waiting. End this turn to restore it.',target:transform,reveal:()=>{setWorkspace('play');return $<HTMLElement>('#persistent-end-turn');}};
        return {title:'Return to Play',copy:'This transformation is unavailable right now. Resolve any pending requirement or choose another legal option before ending the turn.',target:transform,reveal:()=>{setWorkspace('play');return $<HTMLElement>('#open-actions-view');}};
      }
    ...
    const end=$<HTMLButtonElement>('#end-form-button');if(active&&selected?.profile==='base'&&!end.hidden){
      if(!end.disabled)return {title:`Return to ${character.name}`,copy:'Base Form is selected. End the current form when you are ready to return to normal.',target:end};
      if(pendingDamageRolls.size)return {title:'Resolve damage',copy:'A confirmed hit is waiting for its separate damage roll before this turn can finish.',target:end,reveal:revealTask('actions','Roll Damage')};
      if(state.turn.actionsRemaining>0||state.turn.surgeActionsRemaining>0||(state.turn.attackAction?.remaining??0)>0)return {title:'Use your remaining Action',copy:'Ending this form is waiting for a new Bonus Action, but you can still use the Action available this turn.',target:end,reveal:revealTask('actions')};
      if(turnReadyToEnd())return {title:'Finish this turn',copy:'Ending this form needs a new Bonus Action and no other tracked action is waiting. End this turn, then release the form on the next one.',target:end,reveal:()=>{setWorkspace('play');return $<HTMLElement>('#persistent-end-turn');}};
      return {title:'Return to Play',copy:'Ending this form is unavailable right now. Resolve any pending requirement or choose another legal option first.',target:end,reveal:()=>{setWorkspace('play');return $<HTMLElement>('#open-actions-view');}};
    }
  ...
  if(currentWorkspace==='task'){
    if(currentTab==='rolls'&&latestRollTab==='rolls'&&latestRollTitle.startsWith('Initiative')){
      if(state.pendingUncannyMetabolism)return {title:'Consider Uncanny Metabolism',copy:'Initiative is set and Focus Points are missing. You may use this once-per-Long-Rest option now, or continue without it.',target:$<HTMLElement>('#nav-play'),reveal:revealTask('actions','Uncanny Metabolism')};
      return {title:'Initiative is set',copy:'Return to Play and take your turn. Roll Initiative again only if your table asks for a reroll.',target:$<HTMLElement>('#nav-play')};
    }
  ...
  if(currentWorkspace==='play'){
    ...
    if(state.pendingUncannyMetabolism)return {title:'Consider Uncanny Metabolism',copy:'Initiative was rolled with expended Focus Points. You may restore them and heal now, or continue without using the once-per-Long-Rest feature.',target:$<HTMLElement>('#open-actions-view'),reveal:revealTask('actions','Uncanny Metabolism')};
    if(pendingDamageRolls.size)return {title:'Resolve damage',copy:`${pendingDamageRolls.size} confirmed hit${pendingDamageRolls.size===1?' is':'s are'} waiting for a separate damage roll. Resolve or clear ${pendingDamageRolls.size===1?'it':'them'} before ending the turn.`,target:$<HTMLElement>('#open-actions-view'),reveal:revealTask('actions','Roll Damage')};
    ...
    if(state.activeTransform&&classLevel(character,'Barbarian')>0&&!state.rage.active&&!state.concentration&&state.turn.bonusRemaining>0&&(rage?.current??0)>0)return {title:'Consider Rage, then attack',copy:'Rage improves durability and Strength checks or saves while leaving your Action free. Altered’s table rule also applies Rage Damage to eligible Strength-based Beast-form attacks.',target:abilities,reveal:revealTask('features','Start Rage')};
    ...
    const usableBonus=sheet.features.find(feature=>(feature.id==='second-wind'&&!secondWindError(character,state))||(feature.id==='lay-on-hands'&&!layOnHandsError(character,state,1))||(feature.id.startsWith('private-')&&feature.activation==='bonus'&&feature.status!=='inactive'&&!actionCostError(state,'bonus',sheet.conditionImmunities)));
...
function render(){sheet=resolveSheet(character,state);document.documentElement.dataset.alteredCharacter=character.name;syncAuraState();renderCharacterStrip();renderTransformSelector();renderArt();renderMetrics();renderCharacterSnapshot();renderQuickReceivedEffects();renderResources();renderQuickFeatures();renderActiveEffects();renderTab();renderConditions();renderLog();renderNextStepGuide();persist();}

type ImportMethod='ddb'|'json'|'character-pdf'|'owned';
function selectImportMethod(method:ImportMethod){
  const targetId=`import-method-${method}`;
  for(const details of Array.from(document.querySelectorAll<HTMLDetailsElement>('.import-grid>.import-block')))details.open=details.id===targetId;
  return document.querySelector<HTMLDetailsElement>(`#${targetId}`);
}
function openImportCenter(message?:string,method:ImportMethod='ddb'){
  ...
  else setImportStatus('Choose one import method. D&D Beyond is the most complete character source; every result is reviewed before it changes your sheet.');
  const selected=selectImportMethod(pendingDdbImport?'ddb':method);$<HTMLDialogElement>('#import-dialog').showModal();window.setTimeout(()=>selected?.querySelector<HTMLElement>('summary')?.focus({preventScroll:true}),0);
}
...
function initializeControls(){
  setStorageFailureHandler(showStorageWarning);
  ...
  $('#new-user-other-import').addEventListener('click',()=>{$<HTMLDialogElement>('#new-user-character-dialog').close();openImportCenter('Choose Basic character from PDF below, or open Altered character JSON. Nothing replaces the current demo until you confirm it.','character-pdf');});
  ...
  $('#toggle-app-menu').addEventListener('click',()=>{const open=$<HTMLButtonElement>('#toggle-app-menu').getAttribute('aria-expanded')!=='true';setAppMenuOpen(open,!open);});
  ...
  const rollToast=$('#roll-toast');rollToast.addEventListener('click',event=>{if((event.target as Element).closest('.heroic-reroll-controls'))return;dismissRollToast();});rollToast.addEventListener('keydown',event=>{if((event.target as Element).closest('button')||!['Enter',' ','Escape'].includes((event as KeyboardEvent).key))return;event.preventDefault();dismissRollToast();});$('#roll-toast-dismiss').addEventListener('click',event=>{event.stopPropagation();dismissRollToast();});
  ...
  $('#manage-private-content').addEventListener('click',()=>{openImportCenter('Manage private PDFs, structured JSON packs, and quarantined recovery files here.','owned');window.setTimeout(()=>$('#installed-pack-list').scrollIntoView({block:'nearest'}),0);});
  $('#coverage-open-import').addEventListener('click',()=>openImportCenter('Review the character import and add only private mechanics you have verified from an authorized source. PDFs remain references until you explicitly configure a mechanic.','owned'));
  ...
  $('#transform-button').addEventListener('click',()=>{const option=currentOption();if(option){const castingChoice=chosenTransformationCastingOption(option);applyResult(startTransformation(character,state,option,castingChoice));}});
  $('#end-form-button').addEventListener('click',endCurrentForm);
  $('#delete-custom-form').addEventListener('click',()=>{void (async()=>{const control=$<HTMLButtonElement>('#delete-custom-form'),packId=control.dataset.packId,label=control.dataset.formLabel||'this custom form';if(!packId||control.disabled)return;if(!window.confirm(`Delete ${label} from Altered? This removes the private pack that created it.`))return;await removeExtensionPack(packId);installedPacks=await loadValidatedInstalledPacks();rebuildEffectiveCharacterLibrary(true);selectedOptionId='base';renderInstalledPacks();renderSettings();render();notify(`${label} deleted from Altered.`);})();});
  ...
  window.addEventListener('keydown',event=>{if(event.key!=='Escape')return;if(!$('#roll-toast').hidden){event.preventDefault();dismissRollToast();return;}if(document.querySelector('dialog[open]'))return;if(!$('#walkthrough').hidden)finishWalkthrough('Walkthrough closed. Restart it anytime from Help.');else if($<HTMLButtonElement>('#toggle-app-menu').getAttribute('aria-expanded')==='true')setAppMenuOpen(false,true);else if(currentWorkspace!=='play')setWorkspace('play',undefined,taskOrigin,$('#nav-play'));});
  ...
  $('#storage-warning-export').addEventListener('click',()=>downloadJson(character,`${character.name.toLowerCase().replace(/[^a-z0-9]+/g,'-')||'altered-character'}-recovery.json`));
  ...
  $('#owned-pack-file').addEventListener('change',async event=>{const input=event.target as HTMLInputElement;const file=input.files?.[0];if(!file)return;try{if(file.type==='application/pdf'||file.name.toLowerCase().endsWith('.pdf')){selectImportMethod('owned');setImportStatus('That is a PDF. Use “Upload Owned PDF” in this open section; Altered will store the source privately before any review.');$<HTMLInputElement>('#private-pdf-file').focus();return;}const pack=safeOwnedContentParse(await file.text());const result=await installAndApplyPack(pack);setImportStatus(result.applied?`${pack.metadata.name} installed and applied to ${character.name}. ${packCounts(pack)}.`:`${pack.metadata.name} installed. It does not match ${character.name}, so the current sheet was not changed.`);renderSettings();}catch(error){setImportStatus(`JSON rules-pack installation failed: ${error instanceof Error?error.message:'Unknown error'}`);}finally{input.value='';}});
...
async function boot(){
  ...
  const repairNote=invalidPackCount?` ${invalidPackCount} unreadable private pack${invalidPackCount===1?' is':'s are'} quarantined under Import; export or remove ${invalidPackCount===1?'it':'them'} there.`:'';const migrationNote=restoredDataRepairs.length?` ${restoredDataRepairs.join(' ')}`:'';
