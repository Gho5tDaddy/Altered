...
function renderFeatures(){
  ...
  const intro=document.createElement('div');intro.className='feature-page-intro';intro.append(text('strong','What happens automatically?'),text('p','Use the buttons in “Use now” above. These cards distinguish applied rules, conditional rules, owned reference entries, and effects that are unavailable right now.'));const legend=document.createElement('div');legend.className='feature-status-legend';legend.append(statusChip('active','Active · applied now'),statusChip('warning','Conditional · use when triggered'),statusChip('available','Owned · reference'),statusChip('inactive','Unavailable now'));intro.append(legend);root.append(intro);
  ...
  if(sheet.features.length){const activeCount=sheet.features.filter(feature=>feature.status==='active').length;const ownedCount=sheet.features.filter(feature=>feature.source==='Feat'&&feature.status==='ruling').length;const group=featureReferenceSection(`Character features (${sheet.features.length})`,`${activeCount} applied now${ownedCount?` · ${ownedCount} owned feat${ownedCount===1?'':'s'}`:''} · open for explanations`,!sheet.form&&!state.overlays.length);for(const feature of sheet.features){const ownedFeat=feature.source==='Feat'&&feature.status==='ruling';const featureStatus:UiStatus=feature.status==='active'?'active':feature.status==='conditional'?'warning':feature.status==='inactive'?'inactive':ownedFeat?'available':'warning';const badge=feature.status==='active'?'Applied now':feature.status==='conditional'?'Conditional':feature.status==='inactive'?'Unavailable now':ownedFeat?'Owned · reference':'Reference';const c=card(feature.name,badge,`${feature.reason} ${feature.summary}`,featureStatus);c.badge.className=`badge ${ownedFeat?'owned':feature.status}`;group.content.append(c.node);count++;}root.append(group.section);}
