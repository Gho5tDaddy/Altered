import type {Ability,AttackAction,Character,ConditionEffect,CreatureAction,DamagePacket,DamageType,EvaluatedFeature,GameState,OwnedContentMatch,OwnedContentPack,ResolvedSheet,Spell,TransformationEffects,TransformationOption,TransitionResult} from './types.js';
...
import {
  actionCostError,actionExecutionError,applyCondition,applyDamage,attackBonuses,attackRollSources,availableSpellSlotLevels,availableTransformations,boundedWhole,castSpell,criticalDiceExpression,criticalHitThreshold,
  ...
  proficiencyBonus,rageStartError,rollAttackD20,rulesMetadata,shortRest,spellActiveEffect,spendActionCost,spendActionExecution,startNewTurn,startRage,startTransformation,useActionSurge,useLayOnHands,useSecondWind,useWildResurgence,wildResurgenceError
} from './engine.js';
...
function renderActions(){
  ...
  const equipped=character.items.filter(item=>item.equipped&&(!item.requiresAttunement||item.attuned));if(equipped.length&&sheet.profile==='base'){const gear=document.createElement('details');gear.className='feature-reference-section equipped-gear';const summary=document.createElement('summary');const copy=document.createElement('span');const weapons=equipped.filter(item=>item.attack).length;copy.append(text('strong',`Equipped gear (${equipped.length})`),text('small',`${weapons} usable weapon${weapons===1?'':'s'} · imported bonuses are not applied twice`));summary.append(copy);const content=document.createElement('div');content.className='feature-reference-content';for(const item of equipped){const detail=[item.type,item.attack?`${item.attack.damage} ${item.attack.damageType}`:'No separate action',item.requiresAttunement?'Attuned':'',item.mechanics.replaceAll('-',' ')].filter(Boolean).join(' · ');content.append(card(item.name,item.attack?'Weapon ready':'Equipped',detail,item.attack?'available':'active').node);}gear.append(summary,content);root.append(gear);}
  ...
  for(const action of sheet.actions){
    ...
    if(action.type==='attack'){
      ...
      const names=multiattackSequence(action).map(id=>sheet.actions.find(candidate=>candidate.id===id)?.name??id);const c=card(action.name,actionCostLabel(action.cost),`Rolls each component separately with its own attack roll or saving throw: ${names.join(' → ')}. Damage, critical hits, and on-hit effects are shown per component. Optional choices configured on the individual attack cards are applied when eligible.${action.notes?` ${action.notes}`:''}${economyError?` · ${economyError}`:''}`,economyError?'unavailable':'available');if(economyError)c.badge.className='badge inactive';
...
function appendFeatureControls(feature:EvaluatedFeature,actions:HTMLElement){
  if(feature.id==='rage'){
    const error=state.rage.active?null:rageStartError(character,state);const control=button(state.rage.active?'End Rage':'Start Rage · Bonus Action',()=>applyResult(state.rage.active?endRage(state):startRage(character,state)),state.rage.active?'button secondary':'button primary');control.disabled=Boolean(error);if(error)control.title=error;actions.append(control);return;
  }
  if(feature.id==='wild-shape'||feature.id==='large-form'){actions.append(button(feature.id==='wild-shape'?'Choose Form':'Open Forms',()=>setWorkspace('forms'),'button primary'));return;}
  if(feature.id==='wild-resurgence'){
    ...
    const toSlotError=wildResurgenceError(character,state,'shape-to-slot');const toSlot=button('Wild Shape → L1 Slot',()=>applyResult(useWildResurgence(character,state,'shape-to-slot')),toSlotError?'button secondary':'button primary');toSlot.disabled=Boolean(toSlotError);if(toSlotError)toSlot.title=toSlotError;actions.append(toShape,toSlot);return;
  }
  if(feature.id.startsWith('private-')&&feature.activation&&feature.activation!=='none'&&feature.status!=='inactive'){
    const error=actionCostError(state,feature.activation,sheet.conditionImmunities);const use=button(`Use ${feature.name} · ${actionCostLabel(feature.activation)}`,()=>{const failure=spendActionCost(state,feature.activation!,sheet.conditionImmunities);if(failure){notify(failure);render();return;}showRoll('Activated',`${feature.summary} Resolve target or battlefield prerequisites from your authorized source.`,feature.name);render();},error?'button secondary':'button primary');use.disabled=Boolean(error);if(error)use.title=error;actions.append(use);
  }
}
function featureCard(feature:EvaluatedFeature){
  const ownedFeat=feature.source==='Feat'&&feature.status==='ruling';const featureStatus:UiStatus=feature.status==='active'?'active':feature.status==='conditional'?'warning':feature.status==='inactive'?'inactive':ownedFeat?'available':'warning';const badge=feature.status==='active'?'Applied now':feature.status==='conditional'?'Ready / conditional':feature.status==='inactive'?'Unavailable now':ownedFeat?'Owned · reference':'Reference';const c=card(feature.name,badge,'',featureStatus);c.badge.className=`badge ${ownedFeat?'owned':feature.status}`;const paragraph=c.node.querySelector(':scope > p');if(paragraph){paragraph.className='feature-explanation';paragraph.append(text('span','What it does','feature-explanation-label'),text('span',feature.summary),text('span','How to use it','feature-explanation-label'),text('span',featureHowToUse(feature,ownedFeat)));}appendFeatureControls(feature,c.actions);return c.node;
}
function renderFeatures(){
  const root=$('#tab-content');clear(root);let count=0;
  const intro=document.createElement('div');intro.className='feature-page-intro';intro.append(text('strong','Abilities, explained'),text('p','Open a group below. Every entry says what it does and whether you press a button, confirm a trigger, or do nothing because Altered applies it automatically.'));const legend=document.createElement('div');legend.className='feature-status-legend';legend.append(statusChip('warning','Use or confirm'),statusChip('active','Applied automatically'),statusChip('available','Reference only'),statusChip('inactive','Unavailable now'));intro.append(legend);root.append(intro);
  ...
  if(sheet.features.length){const groups=[{title:'Use or confirm',helper:'Abilities you activate and rules with a trigger',features:sheet.features.filter(feature=>feature.status==='conditional'),open:true},{title:'Applied automatically',helper:'Benefits Altered is applying now—no button required',features:sheet.features.filter(feature=>feature.status==='active'),open:true},{title:'Owned references',helper:'Imported names whose private mechanics are not configured',features:sheet.features.filter(feature=>feature.status==='ruling'),open:false},{title:'Unavailable now',helper:'Open to see exactly what currently blocks these features',features:sheet.features.filter(feature=>feature.status==='inactive'),open:false}];for(const entry of groups){if(!entry.features.length)continue;const group=featureReferenceSection(`${entry.title} (${entry.features.length})`,entry.helper,entry.open);for(const feature of entry.features){group.content.append(featureCard(feature));count++;}root.append(group.section);}}
