...
import {
  ...
  proficiencyBonus,rageStartError,rulesMetadata,shortRest,spendActionCost,startNewTurn,startRage,startTransformation,useActionSurge,useLayOnHands,useSecondWind,useWildResurgence,wildResurgenceError
} from './engine.js';
...
function renderActiveEffects(){
  ...
  if(state.concentration){
    const panel=document.createElement('article');panel.className='effect-card concentration-effect';panel.append(text('strong',`Concentrating on ${state.concentration.name}`),text('small','Taking damage queues a Constitution saving throw. Starting Rage or another Concentration effect ends this one.'));cards.push(panel);
  }
...
function renderQuickFeatures(){
  ...
  if(classLevel(character,'Barbarian')>=1){
    const rageError=state.rage.active?null:rageStartError(character,state);const rage=button(state.rage.active?'Rage Active · End':'Start Rage · Bonus Action',()=>applyResult(state.rage.active?endRage(state):startRage(character,state)),'button secondary'+(state.rage.active?' active rage-active-button':''));rage.disabled=Boolean(rageError);if(rageError)rage.title=rageError;box.append(rage);
...
function renderSpells(){
  ...
  const circleInForm=sheet.profile==='wildshape'&&isMoonDruid();root.append(text('p',state.rage.active?'Rage blocks all spells, including Circle of the Moon spells. End Rage before casting.':circleInForm?'While Wild Shaped, your Circle of the Moon spells remain available. Other spells stay listed below so you can see exactly why they are blocked.':'Choose a spell and, when relevant, the spell-slot level. Higher-level slots are valid and scaling is applied for supported spells.','tab-help'));
