...
function createReceivedEffect(){const kind=$<HTMLSelectElement>('#received-effect-kind').value as ReceivedEffectKind,info=RECEIVED_EFFECTS[kind],source=$<HTMLInputElement>('#received-effect-source').value.trim()||'Another creature';const effect:ReceivedEffect={id:`received:${kind}:${Date.now()}`,kind,name:info.name,source,addedTurn:state.turn.number,duration:info.duration,...(kind==='guidance'?{skill:$<HTMLSelectElement>('#received-effect-skill').value}:{}),...(kind==='bardic-inspiration'?{die:Number($<HTMLSelectElement>('#received-effect-die').value) as 6|8|10|12}:{})};applyResult(addReceivedEffect(state,effect));$<HTMLDetailsElement>('#add-received-effect').open=false;}
...
function initializeControls(){
  ...
  const clearPendingDamage=()=>{if(pendingDamageRolls.size){addActivity(`${pendingDamageRolls.size} unresolved damage roll${pendingDamageRolls.size===1?' was':'s were'} cleared at the turn boundary.`);pendingDamageRolls.clear();}},newTurn=()=>{clearPendingDamage();applyResult(startNewTurn(state));},finishTurn=()=>{clearPendingDamage();applyResult(endTurn(character,state));};$('#new-turn').addEventListener('click',newTurn);$('#persistent-new-turn').addEventListener('click',newTurn);$('#end-turn').addEventListener('click',finishTurn);$('#persistent-end-turn').addEventListener('click',finishTurn);$('#short-rest').addEventListener('click',()=>applyResult(shortRest(state)));$('#long-rest').addEventListener('click',()=>applyResult(longRest(character,state)));
